...
Animate( void )
{
	...
	for( i=0, p=Fire; i < NUMFIRES; i++, p++ )
	{
		dt = Time - p->t0;
		p->x = p->x + p->vx * dt;
		p->y = p->y + p->vy * dt + .5 * GRAVITY * dt * dt;
		p->z = p->z + p->vz * dt;
		//p->r = count % 15 == 0 ? 1 : 0;
		//p->g = count % 15 == 5 ? 1 : 0;
		//p->b = count % 15 == 10 ? 1 : 0;
...
InitGraphics( void )
{
	...
	for( i=0, p=Fire; i < NUMFIRES; i++, p++ )
	{
		p->x0 = XMIN + rand() % (int)(XMAX - XMIN);
		p->y0 = YMIN + rand() % (int)(YMAX - YMIN);
		p->z0 = ZMIN + rand() % (int)(ZMAX - ZMIN);
		p->vx = VELMIN + rand() % (int)(VELMAX - VELMIN);
		p->vy = VELMIN + rand() % (int)(VELMAX - VELMIN);
		p->vz = VELMIN + rand() % (int)(VELMAX - VELMIN);
		p->t0 = 0.;
		p->t1 = TMIN + rand() % (int)(TMAX - TMIN);
		p->r0 = (float)rand() / (float)RAND_MAX;
		p->g0 = (float)rand() / (float)RAND_MAX;
		p->b0 = (float)rand() / (float)RAND_MAX;
	}
