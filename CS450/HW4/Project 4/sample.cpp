...
const char *WINDOWTITLE = { "Project 4: Lighting -- Nathaniel Mohr" };
...
Display( )
{
	...
	glLightModelfv(GL_LIGHT_MODEL_AMBIENT, MulArray3(.4, White));
	...
	// Moving, green spot light
	if (!Light1On) {
		glDisable(GL_LIGHT1);
	}
	else {
		SetSpotLight(GL_LIGHT1, sin(36 * Time), cos(36 * Time), 0, sin(36 * Time), cos(36 * Time), 0, 0, 1, 0);
	}
	glColor3f(0., 1., 0.);
	glPushMatrix();
	glTranslatef(sin(36*Time), cos(36*Time), 0);
	glutSolidSphere(.1, 50, 50);
	glPopMatrix();

	// Stationary, pink point light
	if (!Light2On) {
		glDisable(GL_LIGHT2);
	}
	else {
		SetPointLight(GL_LIGHT2, 1, -1, 1, .95, .25, .92);
	}
	glColor3f(.95, .25, .92);
	glPushMatrix();
	glTranslatef(1, -1, 1);
	...
	// Smooth Shaded, Moving, Dull "Sphere"
	glPushMatrix();
	glTranslatef(1., 1., 1.);
	glRotatef(360 * 5* Time, 0, 0, 1);
	glRotatef(90, 1., 0., 0.);
	glScalef(1., 1., 0.5);
	SetMaterial(1., 0., 0., 0.);
	...
	// Smooth shaded, stationary icosahedron
	glPushMatrix();
	glTranslatef(-1., -1., -1);
	SetMaterial(0, 1, 0, .5);
	glShadeModel(GL_SMOOTH);
	...
	// Flat shaded, stationary, shiny cone
	glPushMatrix();
	glTranslatef(-1, -1, 1);
	glRotatef(90, 0, 1, 0);
	SetMaterial(0, 0, 1, 1);
