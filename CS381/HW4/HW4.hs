...
block [] s     = (s, []) 
block (c:cs) s = case cmd c s of 
                     (t, Just l)  -> (fst (block cs t), [l] ++ (snd (block cs t)))
                     (t, Nothing) -> (fst (block cs t),         snd (block cs t))
...
optimize :: Prog -> Prog
optimize []              = []
optimize ((Move x y):cs) = [Move (Lit (expr x)) (Lit (expr y))] ++ (optimize cs)
